...
{   char arquivo[40]; // Char estamos criando o conjunto de banco de dados [40] significa que possui a quantidade máxima de caracteres da variavel(String)
    ...
    file= fopen(arquivo, "w");  // Abre o arquivo e "w" write para escrever a informaçao. 'w' abre ou cria um arquivo de texto no modo de escrita
    fprintf(file, cpf);         // Salvo o valor da variável cpf
    fclose(file);               // Fecha o arquivo
    
    file= fopen(arquivo, "a");  // Acessando o arquivo e "a" para atualizar a informaçao. 'a' - abre um arquivo no modo de inclusão (append)
